...
class EnhancedDeviceRegisterClient:
    ...
    def __init__(self):
        ...
        self.tt_encrypt = TT()  # 创建TTEncrypt实例
    
    def build_headers(self, device_info: Dict[str, Any], ss_stub: str = None) -> Dict[str, str]:
        """构建请求头"""
        headers = {
    ...
    def encrypt_payload(self, payload: Dict[str, Any]) -> bytes:
        ...
        # 使用TTEncrypt加密
        encrypted_data = self.tt_encrypt.encrypt(json_payload)
        
        return encrypted_data
    
    @staticmethod
    def calculate_md5(data: bytes) -> str:
        """计算数据的MD5哈希值，用于x-ss-stub头部"""
    ...
    @classmethod
    def has_valid_registered_identity(cls, device_info: Dict[str, Any]) -> bool:
        if not isinstance(device_info, dict):
            return False
        device_id = cls.normalize_identity(device_info.get("device_id"))
        install_id = cls.normalize_identity(device_info.get("install_id"))
        return (
            device_id.isdigit()
            and install_id.isdigit()
            and int(device_id) > 0
            and int(install_id) > 0
        )
    ...
    def register_device(self, device_info: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # 计算加密数据的MD5哈希值
        ss_stub = self.calculate_md5(encrypted_data)
        
        # 构建请求头和参数
        headers = self.build_headers(device_info, ss_stub)
        ...
        try:
            ...
            response = requests.post(
                ...
                params=params,
                headers=headers,
                data=encrypted_data,  # 发送加密后的二进制数据
                timeout=30,
                verify=True
            )
            ...
            result = {
                ...
                "request": {
                    ...
                    "params": params,
                    "payload": payload,  # 原始未加密的payload
                    "encrypted_data_size": len(encrypted_data),  # 加密后的数据大小
            ...
            try:
                ...
                valid_identity = self.has_valid_registered_identity(device_info)
                result["response"]["valid_device_identity"] = valid_identity
                result["response"]["success"] = response.status_code == 200 and valid_identity
                if result["response"]["success"]:
                    print(f"[SUCCESS] 注册成功! 状态码: {response.status_code}")
                else:
                    print("[WARNING] 响应返回 200，但未拿到有效的 install_id/device_id，视为无效设备")

            except json.JSONDecodeError:
                print(f"[WARNING] 响应不是有效的JSON: {response.text[:100]}...")

            if not result["response"]["success"]:
                print("[WARNING] 设备注册结果无效，不会作为可用设备使用")
...
def save_results(results: List[Dict[str, Any]], xml_configs: List[Dict[str, Any]], 
    ...
    for i, config in enumerate(xml_configs):
        single_xml_file = os.path.join("results", f"device_config_{timestamp}_{i+1}.yaml")
...
def main():
    """主函数"""
    print("批量设备注册工具 (XML配置格式) - TTEncrypt加密版")
    print("=" * 60)
